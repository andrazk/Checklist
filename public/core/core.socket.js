...
core.socket = new io.Socket();

core.socket.online = false;
core.socket.connect();
core.socket.on('connect', function(){
	core.socket.online = true;
	if (core.notify) core.notify('socketConnected');
	if(core.online) core.sync.sync();
...
core.socket.on('disconnect', function(){
	core.socket.online = false;
	core.notify('socketDisconnected');
