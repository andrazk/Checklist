...
core.sync = function(){
	return {
		...
		loadEntity: function(log, cb){
			...
			if(log.action == 'delete') return cb('izbris', null);
			var entity;
			switch(log.type){
				case 'Project':
		...
		one: function(slog, sentity){
			var self = this;
			if(!slog) return console.log('sLog nima IID');
